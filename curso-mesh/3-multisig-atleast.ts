import { MeshTxBuilder, MeshWallet, NativeScript, Transaction, UTxO, resolveNativeScriptAddress, resolveNativeScriptHex, resolvePaymentKeyHash } from "@meshsdk/core";
...
// También tiene que ser alguna
const addrWalletEmisora = addresses[0];

const atLeastScript: NativeScript = {
    type: "atLeast",
    required: 3,
    scripts: addresses.map(
        (address) => {
            return {
                type: "sig",
...
// console.log(atLeastScript);
const scriptAddress = resolveNativeScriptAddress(atLeastScript, 0);
...
const tx = new Transaction({initiator: walletEmisora}).sendLovelace(receptor, "909999").setChangeAddress(addrWalletEmisora);
...
const hashDeTransaccionEnviada = "42666be77c838211fa08a1c8d47b14b43e7c57299c2a3f5f0f909fc78d3be593";
...
async function redeem() {
    ...
    const redeemTx = new MeshTxBuilder({})
        ...
        .txInScript(resolveNativeScriptHex(atLeastScript))
